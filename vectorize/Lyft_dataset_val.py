# This file is implemented based on https://github.com/Kguo-cs/ccil/blob/main/data/Lyft_dataset.py.
import numpy as np
import math
import torch
import bisect
...
class LyftDataset_val(Dataset):
    def __init__(self, cfg, type, meta_manager, zarr_dataset, map_api,scene_dataset=False):
    ...
    def __getitem__(self, index) -> dict:
        first_path=str(index//10000)
        if index<10000:
            second_path=str(index)
        else:
            second_path=str(index%10000)
        store_path='./10_data/'+self.type+'/'+first_path+'/'+second_path+'.pkl'
        if os.path.exists(store_path) and os.path.getsize(store_path):
    ...
    def get_frame(self, lane_df,left_graph, right_graph, scene_index: int, frame_index: int, dataset, selected_track_id: Optional[int] = None):
        ...
        if len(future_frames)==0:
            ...
            left_dict = nx.to_dict_of_lists(left_graph)
            right_dict = nx.to_dict_of_lists(right_graph)
            ...
            if contain_lanes.size != 0:
                ...
                if len(lane_coords)==0:
                    ...
                    if len(future_frames)>1:
                        ...
                        tar_pos=history_frames[0]["ego_translation"][:2]
    ...
    def get_scene_dataset(self, scene_index: int) :
        ...
        return LyftDataset_val(self.cfg, 'val', self.meta_manager, dataset, True)
