...
class LyftManager(MapManager):
    def __init__(self, cfg, dm):
        ...
        try:
            print("load map data")
            vectormap = np.load("./vectorize/vectorized_data/lyft_map_features.npz", allow_pickle=True)
            ...
            np.savez_compressed('.vectorize/vectorized_data/lyft_map_features',
    ...
    def get_scenetargets(self ,type, frames, cumulative_sizes,goal_num=20):
        try:
            goal = np.load("./vectorize/vectorized_data/" + type + "_lane_goal20.npy")
            ...
            np.save("./vectorize/vectorized_data/" + type + "_lane_goal20.npy",goal_array)
    ...
    def build_graphs(self):
        ...
        dist_graph = self.build_dist_graph(nei_graph)  # np.load("./vectorized_data/lyft_map_features.npz",allow_pickle=True)['dist_graph']#
