...
class ObsEncoder(nn.Module):
    ...
    def forward(self,batch) :
        ...
        if crosswalk_polylines.max()==0:
            batch_dim=ego_features.shape[0]
            hidden_dim=ego_features.shape[-1]
            cross_features=torch.zeros((batch_dim,12,hidden_dim)).to(device)
            cross_mask=torch.zeros((batch_dim,12)).to(bool).to(device)
        else:
