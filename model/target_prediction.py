...
from model.utils import masked_softmax
from model.basic_module import MLP
...
class TargetPred(nn.Module):
    def __init__(self,
        ...
        self.prob_mlp = nn.Sequential(
            MLP(in_channels + 3, hidden_dim, hidden_dim),
    ...
    def forward(self, feat_in: torch.Tensor, tar_candidate: torch.Tensor, candidate_mask=None):
        ...
        feat_in_repeat = torch.cat([feat_in.repeat(1, int(n/m), 1), tar_candidate], dim=2)
        feat_in_repeat=feat_in_repeat.type(torch.float32)
        ...
        if not isinstance(candidate_mask, torch.Tensor):
            ...
            tar_candit_prob = masked_softmax(prob_tensor, candidate_mask.squeeze(2), dim=-1)               # [batch_size, n_tar]
        value,indices=tar_candit_prob.topk(20, dim=1)
