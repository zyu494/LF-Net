...
def Loss(pred_dict, gt_dict, aux_pred=None, aux_gt=None):
        """
        ...
        device=pred_dict['target_prob'].device
        loss = 0.0

        # pos_weight=torch.tensor([2399/1]).to(device)
        # cls_loss=F.binary_cross_entropy_with_logits(pred_dict['target_prob'], gt_dict['target_prob'].float(),
        ...
        placeholder=torch.zeros(1,3).to(device)
        for id in check_list:
             offset=torch.cat([offset[:id],placeholder,offset[id:]],0)
